"""Generation by Main Fuel Group dashboard page."""
...
from components.charts import (
    build_gen_mix_charts, group_fuel_rows,
    GROUP_COLORS, GROUP_ORDER,
)
...
with cols[1]:
    from_date = st.date_input(
        ...
        key=f"mfg_from_date_{granularity}",
        label_visibility="collapsed",
    )
with cols[2]:
    if show_time:
        from_time = st.selectbox(
            ...
            key=f"mfg_from_time_{granularity}",
            label_visibility="collapsed",
        )
    else:
        from_time = "00:00"
...
with cols[4]:
    to_date = st.date_input(
        ...
        min_value=_picker_min_date,
        max_value=_picker_max_date,
        key=f"mfg_to_date_{granularity}",
        label_visibility="collapsed",
    )
with cols[5]:
    if show_time:
        to_time = st.selectbox(
            ...
            key=f"mfg_to_time_{granularity}",
            label_visibility="collapsed",
        )
    else:
        to_time = "23:30"
...
rows = group_fuel_rows(rows, granularity)
...
# ── Build charts ───────────────────────────────────────────────────────────────
figures = build_gen_mix_charts(
    rows,
    granularity,
    window_label,
    color_map=_GROUP_COLORS,
    fuel_order_override=GROUP_ORDER,
)

# Override titles for the grouped context
_CHART_TITLES = [
    "Generation by Main Fuel Group — Great Britain",
    "Share by Fuel Group",
    "Fuel Groups — Line View",
    "Generation Share by Fuel Group (%)",
]
figures = [(_CHART_TITLES[i], fig) for i, (_, fig) in enumerate(figures)]

# ── Chart headers ─────────────────────────────────────────────────────────────
SUBTITLES = [
    f"Generation output by main fuel group · {window_label}",
    f"Share of generation by fuel group (100% stacked) · {window_label}",
    f"Individual fuel group lines + dashed total · {window_label}",
    f"Individual fuel group share lines · {window_label}",
]
